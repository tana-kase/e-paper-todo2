...
|------|-----|
| cron間隔 | 毎分 (`* * * * *`) |
| 理由 | 即時フィードバックが重要。タスク完了→1分以内に反映 |

### フルリフレッシュ方式

```python
def should_full_refresh() -> bool:
    """5分に1回フルリフレッシュ（ゴースト防止）"""
    return datetime.now().minute % 5 == 0
```

| 時刻（分） | 動作 |
|-----------|------|
| 0, 5, 10, 15... | `clear_display()` + `display_image()` |
| それ以外 | `display_image()` のみ |

### 採用理由

1. **ステートレス**: カウンターファイル不要、時刻だけで判定
2. **シンプル**: 状態管理なしで実装が簡潔
3. **予測可能**: いつフルリフレッシュするか明確
...
```
uploads/ に手動配置 → main.py実行 → images/ に変換済み保存 → 元ファイル削除
```
...
| 項目 | 仕様 |
|------|------|
| 入力フォーマット | PNG, JPG, JPEG, GIF, BMP |
| 出力サイズ | 480x800px |
| グレースケール | 4階調（0, 85, 170, 255） |
| アスペクト比違い | レターボックス（白余白追加） |
| 小さい画像 | 拡大してフィット |
| ファイル名 | 元の名前を維持（拡張子は.png） |

### 採用理由

1. **シンプル**: 別コマンド不要、通常フローで自動変換
2. **即反映**: 次のcron実行（最大1分後）で変換される
3. **手動アップロード前提**: SCPやUSBでuploads/に配置
