...
- [x] **6. HTMLテンプレート作成（基本構造）**
  - Jinja2テンプレートで`templates/board.html`作成
  - 受け入れ条件: ダミーデータでHTMLファイルが生成され、ブラウザで表示できる

- [x] **7. CSSスタイリング（仕様準拠）**
  - 480x800px（縦長）、Zen Kurenaido 20px、下線区切り（近接デザイン）
  - 受け入れ条件: ブラウザで開いて仕様通りのデザインが確認できる

- [x] **8. テキスト省略処理**
  - CSS `text-overflow: ellipsis` で1行強制、自動省略
  - 受け入れ条件: 長いタスク名が適切に省略される

- [x] **9. 最終更新時刻の表示**
  - 画面下部に`MM.DD HH:MM`形式（JST）で表示
  - 受け入れ条件: HTMLに現在時刻が正しいフォーマットで表示される

- [x] **10. WeasyPrintでPNG変換**
